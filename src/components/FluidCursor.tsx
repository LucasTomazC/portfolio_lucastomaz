...
export const FluidCursor: React.FC<FluidCursorProps> = ({
  ...
  useEffect(() => {
    ...
    if (!ext.formatRGBA || !ext.formatRG || !ext.formatR) {
      console.warn("Supported texture formats not found.");
      return;
    }
    ...
    return () => {
      ...
      if (gl) {
        ...
        deleteProgram(clearProgram);
        deleteProgram(splatProgram);
        deleteProgram(advectionProgram);
        deleteProgram(divergenceProgram);
        deleteProgram(curlProgram);
        deleteProgram(vorticityProgram);
        deleteProgram(pressureProgram);
        deleteProgram(gradienSubtractProgram);
        
        if (displayMaterial && displayMaterial.programs) {
          for (const key in displayMaterial.programs) {
            const prog = displayMaterial.programs[key];
